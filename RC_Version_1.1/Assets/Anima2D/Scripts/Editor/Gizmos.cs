					...
					if(!IsLocked(bone))
					{
						...
						if(eventType == EventType.MouseDrag)
						{
							   ...
							   Event.current.button == 0)
							{
								...
								Plane plane = new Plane(bone.transform.forward,bone.transform.position);
								float distance;
								if(plane.Raycast(ray,out distance))
								{
									Vector3 localScale = bone.transform.localScale;
									Vector3 direction = bone.transform.InverseTransformPoint(ray.GetPoint(distance));
									float angle = AngleAroundAxis(Vector3.right, direction, Vector3.Cross(Vector3.right * localScale.x, Vector3.up * localScale.y));
									
									Undo.RecordObject(bone.transform, "Rotate");
									bone.transform.Rotate(0f,0f,angle);
							...
							if(Tools.current == Tool.Move)
							{
								...
								if(EditorGUI.EndChangeCheck())
								{
									...
									if(control.bone)
									{
										...
										BoneUtils.UpdateLinkedParentBone(control.bone,false,"Move",true);
