...
namespace Anima2D 
{
	public class InspectorEditor : WindowEditorTool
	{
		...
		Vector2 GetWindowSize()
		{
			...
			if(spriteMeshCache.isBound && spriteMeshCache.selectedNodes.Count > 0)
			{
		...
		public override void OnWindowGUI(Rect viewRect)
		{
			...
			windowRect.position = new Vector2(5f, viewRect.height - windowRect.height - 5);
		...
		bool IsMixedBoneIndex(int weightIndex, out int boneIndex)
		{
			...
			spriteMeshCache.GetBoneWeight(spriteMeshCache.selectedNodes[0]).GetWeight(weightIndex, out boneIndex, out weight);
		...
		{
			if(spriteMeshCache.selectedNodes.Count > 0)
			{
				...
				EditorGUI.BeginChangeCheck();
				boneWeight = EditorGUIExt.Weight(boneWeight,0,names,mixedBoneIndex0,mixedWeight);
				changedIndex0 = EditorGUI.EndChangeCheck();

				EditorGUI.BeginChangeCheck();
				boneWeight = EditorGUIExt.Weight(boneWeight,1,names,mixedBoneIndex1,mixedWeight);
				changedIndex1 = EditorGUI.EndChangeCheck();

				EditorGUI.BeginChangeCheck();
				boneWeight = EditorGUIExt.Weight(boneWeight,2,names,mixedBoneIndex2,mixedWeight);
				changedIndex2 = EditorGUI.EndChangeCheck();

				EditorGUI.BeginChangeCheck();
				boneWeight = EditorGUIExt.Weight(boneWeight,3,names,mixedBoneIndex3,mixedWeight);
