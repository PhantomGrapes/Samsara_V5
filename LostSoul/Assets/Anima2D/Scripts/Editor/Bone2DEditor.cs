...
namespace Anima2D
{
	...
	public class Bone2DEditor : Editor
	{
		...
		override public void OnInspectorGUI()
		{
			...
			if(childChanged)
			{
				BoneUtils.UpdateLinkedParentBone(m_Bone.child,true,"set child",!AnimationMode.InAnimationMode());
			}
		}
		...
		void OnSceneGUI()
		{
			if(Tools.current == Tool.Move)
			{
				...
				if(EditorGUI.EndChangeCheck())
				{
					...
					BoneUtils.UpdateLinkedParentBone(m_Bone,false,Undo.GetCurrentGroupName(),!AnimationMode.InAnimationMode());
